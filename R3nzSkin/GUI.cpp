...
inline static void footer() noexcept
{
	...
	static const auto buildText{ "Ultima actualizacion: "s + __DATE__ + " - " + __TIME__ };
	ImGui::Separator();
	ImGui::textUnformattedCentered(buildText.c_str());
	ImGui::textUnformattedCentered("Copyright (C) 2023 KONN0R-DEV");
}
...
void GUI::render() noexcept
{
	...
	ImGui::Begin("Zapphire Skin", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_AlwaysAutoResize);
	{
		ImGui::rainbowText();
		if (ImGui::BeginTabBar("TabBar", ImGuiTabBarFlags_Reorderable | ImGuiTabBarFlags_FittingPolicyScroll | ImGuiTabBarFlags_NoTooltip)) {
			if (player) {
				if (ImGui::BeginTabItem("Skins")) {
					...
					ImGui::Text("Opciones de Skins:");

					if (ImGui::Combo("Skin actual", &cheatManager.config->current_combo_skin_index, vector_getter_skin, static_cast<void*>(&values), values.size() + 1))
					...
					}; it != cheatManager.database->specialSkins.end())
					{
						...
						if (ImGui::Combo("Skin Actual", &gear, vector_getter_gear, static_cast<void*>(&it->gears), it->gears.size())) {
